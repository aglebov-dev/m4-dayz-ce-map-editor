...
def parse(data: bytes) -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
    ...
    def skip_array(pos: int) -> int:
        ...
        for _ in range(n):
            ...
            if t == 0:                       # string
                _, pos = asciiz(pos)
            elif t in (1, 2):                # float / int
                pos += 4
            elif t == 3:                     # вложенный массив
                pos = skip_array(pos)
            else:
                pos += 4
        return pos
    ...
    def parse_body(pos: int, cls: str) -> None:
        ...
        for _ in range(n):
            ...
            if etype == 0:                   # вложенный класс: имя + смещение тела
                ...
                else:                        # float / int — 4 байта
                    pos += 4
            elif etype == 2:                 # массив
                _, pos = asciiz(pos)
                pos = skip_array(pos)
            elif etype in (3, 4, 5):         # extern/delete class — только имя
                _, pos = asciiz(pos)
            else:
    ...
    parse_body(16, "")                       # тело корневого класса — сразу за 16-байтным заголовком
