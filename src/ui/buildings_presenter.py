...
STACK_RADIUS_M = 2.0                              # радиус «стека» зданий в точке (высотки)
...
class BuildingsPresenter(QObject):
    ...
    def __init__(self, parent, *, view, colors: LayerColors, settings,
        ...
        self._corners = None                     # (M,4,2) контуры ВСЕХ зданий с footprint
        self._kept = None                        # глобальные индексы этих контуров
        self._corner_row: dict[int, int] = {}    # глобальный индекс -> строка в _corners
        self._built: set[str] = set()            # слои, чьи оверлеи (точки+контур) построены
        self._layer_on: dict[str, bool] = {}     # тогл-состояние слоёв
        self._cands: dict[int, dict] = {}        # инфо-дикты кандидатов текущего клика

        # --- панель слоёв (режим + прозрачности + список) ---
        ...
        # --- инспектор здания (+ переключатель кандидатов) ---
    ...
    def populate(self, areaflags, model, index) -> None:
        ...
        rows = []
        if model and index:
            # контуры всех зданий один раз — и для рендера по слоям, и для hit-теста
    ...
    def show_building_at(self, x: float, z: float, areaflags, colors: dict,
        ...
        if not cands:                            # инспектор очистит подсветку/спавн
    ...
    def _pick(self, x: float, z: float) -> list[int]:
        ...
        near = model.nearest(x, z, self._af)     # ближайшая точка (fallback / режим точек)
        if near is None:
            return []
