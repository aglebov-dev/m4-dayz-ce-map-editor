...
class LayersPresenter(QObject):
    ...
    def __init__(self, parent, *, view, colors: LayerColors, settings,
        ...
        self._built: set[str] = set()            # какие пиксмапы уже построены
        self._auto_shown: str | None = None      # слой, показанный ВЫБОРОМ (гаснет при смене)
        self._suppress_auto = False              # идёт наш программный setChecked, не пользователь

        self.dock = QDockWidget(tr("dock.layers"), parent)
        self.dock.setObjectName("dock_layers")
        self.dock.setWidget(wrap(self.panel))

        # ВСЯ проводка панели — здесь, а не в MainWindow
        ...
        self.panel.layer_selected.connect(self._bus.layer_selected)   # выбор -> на шину
        self._bus.layer_selected.connect(self.auto_show)              # выбор -> временно показать
        self._bus.layer_toggle_requested.connect(self._apply_toggle)  # запрос тогла от других панелей
    ...
    def _on_toggled(self, key: str, visible: bool) -> None:
        ...
        if not self._suppress_auto and key == self._auto_shown:
            self._auto_shown = None              # пользователь тронул тогл — слой теперь его
        if visible:
            self.ensure_built(key)
        self._view.set_overlay_visible(key, visible)
        self._bus.layer_toggled.emit(key, visible)   # зоны/инспекторы синхронятся снаружи
    ...
    def auto_show(self, key: str) -> None:
        ...
        if previous and previous != key:
            ...
            if row and row.btn.isChecked():
                self._suppress_auto = True       # это не пользователь — метку не сбрасываем
    ...
    def _build(self, key: str) -> None:
        ...
        z = (10 + bit) if kind == "tier" else (20 + bit)     # старший флаг поверх
    ...
    def apply_color(self, key: str, rgb: tuple[int, int, int]) -> None:
        ...
        if key in self._built:                   # перекрасить уже построенный оверлей
            self._build(key)
        self._bus.layer_color_changed.emit(key, rgb)   # напр. подписи зон того же слоя
