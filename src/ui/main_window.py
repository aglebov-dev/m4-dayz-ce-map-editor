...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.layers_panel = self.layers.panel
        self.dock_layers = self.layers.dock

        self.buildings_feature = BuildingsPresenter(
            self, view=self.view, colors=self.colors, settings=self.settings,
            mission=self.current_mission, bus=self.bus, wrap=self._dock_widget)
        self.objects = self.buildings_feature
        self.loot_panel = self.buildings_feature.loot_panel
        ...
        self.territories_panel = self.territories.panel
        ...
        self.inspector_panel = self.inspector.panel
        self.dock_inspector = self.inspector.dock
        self.view.clicked_world.connect(self.on_map_clicked)

        self.stats_panel = StatsPanel(self)
        self.stats_panel.select_toggled.connect(self.on_select_toggled)
        self.stats_panel.flag_clicked.connect(self.bus.layer_selected)
        ...
        self.view.erase_toggle_requested.connect(self._toggle_erase)
        self.bus.layer_color_changed.connect(self.brush_panel.set_layer_color)
    ...
    def closeEvent(self, ev):
        if getattr(self, "_dirty_cells", 0):
    ...
    def load_workdir(self, d: str, mission_name: str = "", silent: bool = False):
        d = os.fspath(d)
        self.missions = scan_workdir(d, mission_name)
    ...
    def load_areaflags(self, m: Mission):
        ...
        self.buildings = model.buildings if model else None
        ...
        self.layers.populate(af)
        self.buildings_feature.populate(af, model, self.building_index)
    ...
    def on_brush_mode(self, on: bool):
        ...
        if on:
            self.stats_panel.select_switch.setChecked(False)
