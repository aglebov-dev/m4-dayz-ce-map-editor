...
class ZonesPresenter(QObject):
    ...
    def __init__(self, parent, *, view, colors: LayerColors, bus: AppBus,
        ...
        self._view = view
        self._colors = colors
        self._areaflags = areaflags               # callable -> AreaFlags | None
        self._is_visible = is_layer_visible       # callable(key) -> bool
        self._cache: dict[str, list] = {}
        self._key: str | None = None              # чьи зоны показать, когда панель откроют
        ...
        # тогл слоя из панели зон -> на шину -> применит презентер слоёв (единый источник)
    ...
    def _on_layer_selected(self, key: str) -> None:
        ...
        if not areaflags or key.startswith("obj:"):   # у объектов зон нет
            return
        self._key = key
        # панель зон САМА НЕ ОТКРЫВАЕТСЯ: закрыта — не платим за find_zones, посчитаем при показе
    ...
    def _show(self, key: str) -> None:
        ...
        if zones is None:
            name = key.split(":", 1)[1]
            zones = find_zones(areaflags.plane(name), min_cells=2)   # одиночные ячейки — шум
    ...
    def _on_color_changed(self, key: str, rgb: tuple[int, int, int]) -> None:
        if key == self.panel.layer_key:          # подписи зон — в цвете своего слоя
